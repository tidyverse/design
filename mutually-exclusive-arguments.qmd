...
Leave the mutually exclusive arguments free from defaults then use `rlang::check_exclusive()`:
...
If you don't want to take a dependency on rlang, you can perform the check "by hand" with use `xor()` and `missing()`:
