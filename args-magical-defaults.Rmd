...
*   `readr::read_csv()` has `progress = show_progress()`, but until version
    XYZ `show_progress()` was not exported from the package. That means if you 
...
*   In packages, it's easy to use a non-exported function. This is available to
    you, the package author, but not the user of the package.
...
This modification expands the interface of the function, because all existing code will work, but `NULL` inputs will now also work (where they probably didn't before). This means that this change should not break existing code. 

The only exception is functions like `data.frame()` where `NULL` is a permissible value, and it does something different to the default:
...
To remediate a function of this nature, you'll need to use a sentinel object, as described in Section \@ref(args-default-sentinel).
