...
It computes the width by first looking at the data, then in the paramters, finally falling back to computing it from the resolution of the `x` variable:
...
Don't use `%||%` for more complex examples.
...
Should the default always be `NULL` and we have a special value to use when you actually want a `NULL`?
