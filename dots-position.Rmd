...
If you've already published a function where you've put `...` in the wrong place, it's easy to fix. You'll need to use `rlang::check_dots_used()` to check that `...` is as expected (cf. Chapter \@ref(dots-inspect)). Since using the full names for details arguments is good practice, making this change will typically affect little existing code, but it is an interface change so should be advertised prominently.
