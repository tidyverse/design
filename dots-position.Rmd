...
* When coupled with "inspect the dots" (Chapter \@ref(dots-inspect)), 
  ...
  misspelled argument names will silently go astray.
...
Following this pattern is simple: just identify which arguments are data and which are details, and then put the `...` in between.

## How do I remediate it?

If you've already published a function where you've put `...` in the wrong place, it's easy to fix. You'll need to use `rlang::check_dots_used()` to check that `...` is as expected (cf. Chapters \@ref(dots-inspect) and \@ref(dots-data)). Since using the full names for details arguments is good practice, making this change will typically affect little existing code, but it is an interface change so should be advertised prominently.
