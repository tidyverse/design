...
## What's side-effect soup?

Side-effect soup occurs when you mix side-effect and regular computation. 
...
*   If a function does some computation and has side-effects, it can be 
    challenging to extract the results of computation. For example, 
    `print.summary.lm()` and `plot.lm()`. These functions are frustrating
    because you can see what you want but you can't touch it!
...
1.  Instead of `n` named arguments, they can take a single named list:
    ...
    future, as discussed in Section \@ref(vectors-not-dots))
