...
-   Set a single default and supply the possible values to `match.arg()`:
    ...
    The downside of this approach is that you can no longer see which values are permitted, and you'd have to describe them separately in the documentation.
    You can, however, still see the default value in the function specification.
...
-   Store the options in an exported function, and use it in the defaults:
    ...
    The downside of this approach is that when looking at the function spec, you can no longer easily see the default value, or the set of possible values.
    However, the possible values can easily be found programmatically.

    This is more worthwhile if you want to share the permitted values across multiple functions.
