...
*   `lm()` does not have defaults for `formula`, `data`,
    `subset`, `weights`, `na.action`, or `offset`. But only
    `formula` is actually required, although the error you
    get if you omit it is not the usual.

*   `var()`

*   In `diag()`, the argument `x` has a default `1`, but the function errors if 
    you leave it off:
...
## One complex argument vs two simple arguments {#args-compound}
