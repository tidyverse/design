...
*   In `var()`, `use` has no default, but is optional: it's automatically 
    calculated based on the `na.rm` argument.
...
## When is it ok to violate this pattern?
...
Only use this technique if there are exactly two possible arguments. If there are more than two, that is generally a sign you should create more functions. You can see an example of this in `forcats::fct_lump()`. There are many different ways to decided how to lump uncommon factor levels together, and initially I attempted to encode these through arguments to `fct_lump()`. However, over time as the number of arguments increased, it gets harder and harder to tell what the options are. Now, we're growing `fct_lump()` capabilities by adding new functions, like `fct_lump_min()`.
