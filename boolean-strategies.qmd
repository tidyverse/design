# Prefer a enum, even if only two choices {#sec-avoid-boolean-strategies}
...
-   It's very clear what both `TRUE` and `FALSE` will mean just from the name of the argument. Otherwise, the `TRUE` value tends to be easier to understand because `something = TRUE` implies exactly what will happen, where as `something = FALSE` only tells you what won't happen.
...
-   By default, `stringr::str_subset(string, pattern)` returns the elements of `string` that match the `pattern`. You can use `negate = TRUE` to instead return the elements that don't match the pattern, but I now think this would be more clear as `return = c("matches", "non-matches")`.
...
-   The `decreasing` argument is used to pick between sorting in ascending or descending order.
    It's easy to understand what `decreasing = TRUE` does, but it's not so clear what `decreasing = FALSE`, the default, means:
