...
Firstly, I prefer camelCase to snake_case, so I'd call it `set_names()`. Then we need to consider the arguments:
...
Then we use in `set_names()`, replacing the existing `set_names()`:
