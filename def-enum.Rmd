...
This technique is a best used when the set of possible values is short. You can see that it's already getting unwieldy in `rank()`. The easiest way to resolve this problem is to follow Chapter \@ref(def-short) and extract the possible values into their own function:
...
This is nowhere near as good as inlining all possible values, since you can can no longer see the possible values in tooltips, but you can still access programmatically.
