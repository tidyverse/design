...
We identify three nested levels of naminess that are practically useful:
  ...
  * Unique: No element of `names` appears more than once. A couple specific names are also repaired, such as `""` (the empty string).
...
**Unique** names meet the requirements for minimal and have no duplicates. In the tidyverse, we go further and repair a few specific names: `""` (the empty string), `...` (R's ellipsis or "dots" construct), and `..j` where `j` is a number.
...
names(as_tibble(df, .name_repair = "unique"))
```

Two dots followed by a number, `..j`, even if quoted, cannot always be used. This can be fixed by prepending another dot.
...
Three dots, the ellipsis or "dots" construct, even if quoted, cannot always be used. This is treated like the empty string `""`.
