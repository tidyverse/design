...
Here we address how to manage the `names` attribute of an object. Current content reflects recent thinking about how to handle the column or variable names of a tibble. This may inform a name-handling strategy for vectors, in general. The name repair described below is exposed to users via the `.name_repair` argument of [tibble::tibble()](https://tibble.tidyverse.org/reference/tibble.html) and [tibble::as_tibble()](https://tibble.tidyverse.org/reference/as_tibble.html). The workhorse functions are not (yet) exported. *describes tibble > 1.4.2*

These are the kind of names we're concerned with:
...
This appears to be a useful baseline requirement, if the `names` attribute of an object is going to be actively managed. Why? General name handling and repair can be implemented more simply if the baseline strategy ensures that `names(x)` returns a character vector of the correct length.
...
**Unique** names meet the requirements for minimal and have no duplicates. In the tidyverse, we go further and also assert that unique names are never empty (literally, no `""`s).
...
This augmented definition of "unique" has an important practical property: it ensures that each element can be identified by name. This is especially meaningful for variable names inside a tibble, because so many workflows rely on indexing by name. Note that uniqueness refers implicitly to a vector of names. It is not a property of an individual name.
...
Why would you ever want a tibble with minimal names, instead of unique? Sometimes the first row of a data source -- allegedly variable names --
...
In the tidyverse, when making names syntactic, we often go further and also make sure they are unique. Why? In practice, if you want syntactic names, you probably also want them to be unique.

### Making names syntactic

There are many ways to fix a non-syntactic name. Here's how our logic
compares to [base::make.names()](https://stat.ethz.ch/R-manual/R-patched/library/base/html/make.names.html) for a single name:
 ...
 * Different: We treat `NA` and `""` the same: both become `.`. This because we
   first make names minimal. [base::make.names()](https://stat.ethz.ch/R-manual/R-patched/library/base/html/make.names.html) turns `NA` in `"NA."` and `""` into `"X"`.
