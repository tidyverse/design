...
y <- rbind(c("original", "unique-ified"), x[, 1:2])
glue::glue_data(ascii_table(y), "{original} {unique}")
```
...
This level of naminess is regarded as normative for a tibble and a user must expressly request a tibble with names that violate this (but that is possible).
...
glue::glue_data(ascii_table(y), "{original} {unique} {base}")
```
...
glue::glue_data(ascii_table(y), "{suffixed} {resuffixed}")
```
...
y <- y %>% 
  ascii_table()
  #mutate(anno = ifelse(grepl("[.][.][.0-9]", base_syn), "<- not syntactic", NA))
glue::glue_data(ascii_table(y), "{original} {tidy_syn} {base_syn}")
```
