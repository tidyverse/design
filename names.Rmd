...
This augmented definition of "unique" has an important property: it ensures that each element can be identified by name (literally, `x["name"]` works). This has practical significance for variable names inside a tibble, because so many workflows rely on indexing by name. Note that uniqueness refers implicitly to a vector of names.
...
Why would you ever want a tibble with minimal names, instead of unique? Sometimes the first row of a data source -- allegedly variable names -- actually contains **data** and the resulting tibble will be reshaped with, e.g., `tidyr::gather()`. In this case, it is better to not munge the names at
import.
...
Syntactic names are easy to use "as is" in code. They do not require quoting and work well with nonstandard evaluation, such as list indexing via `$` or in packages like dplyr and ggplot2.
...
In the tidyverse, when making names syntactic, we often go further and also make sure they are unique. Why? In practice, if you want syntactic names, you probably also want them to be unique. We call names that are unique (group-wise property) and syntactic (element-wise property) *semantic*. This is the meaning of `.name_repair = "semantic"` in tibble: user is requesting names that are unique and syntactic. *describes tibble > 1.4.2*
 ...
 * Different: We treat `NA` and `""` the same: both become `.`. This is because
   we first make names minimal. [base::make.names()](https://stat.ethz.ch/R-manual/R-patched/library/base/html/make.names.html) turns `NA` in `"NA."` and `""` into `"X"`.
...
### Making names semantic

TO FILL IN: how exactly we make names unique and syntactic. Is it a simple composition of two primitive operations?
