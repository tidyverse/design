...
We identify three nested levels of naminess that are practically useful:
  ...
  * Unique: No element of `names` is `""` or a duplicate.
    - `x["name"]` works for each `"name"`.
  * ?Semantic?: The `names` are unique and syntactic.
    - `x$name` works for each `"name"`.
...
Why would you ever want to import a tibble and enforce only minimal names, instead of unique? Sometimes the first row of a data source -- allegedly variable names -- actually contains **data** and the resulting tibble will be reshaped with, e.g., `tidyr::gather()`. In this case, it is better to not munge the names at
import.
...
*we are questioning this use of "semantic" (meaning "unique and syntactic") and welcome alternative suggestions*
