...
*  When installing packages, `install.packages()` informs of the value of the
   `lib` argument, which defaults to `.libPath()[[1]]`:
    ...
    refer to the mechanism that controls it: `.libPaths()`.
...
common_by <- function(x, y) {
  ...
  message("Computing default: `by = ", rlang::expr_text(common), "`")
  common
}
...
You can use the function directly as the default value, `by = common_by(x, y)`, which is most appropriate if you want to export and document it. Otherwise, to avoid creating a magical default (Chapter \@ref(args-magical-defaults)), use the technique of Section \@ref(arg-short-null):
