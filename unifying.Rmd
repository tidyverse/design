...
While the subsequent chapters will get deeper into technical details, to begin I wanted to lay out some of the overarching principles that apply throughout the tidyverse: the tidyverse is __human centered__, __consistent__, __composable__, and __inclusive__. 

These principles are aspirational, and the tidyverse doesn't always fully apply them, but we continue to strive towards them. These principles are inspired by writings about the design of other systems, such as:

* [The Unix philsophy][unix-philosophy]
* [The Zen of Python][zen-python]
* [Design Principles Behind Smalltalk][smalltalk-design]

The primary goal of the tidyverse is to facilitate the conversation that a human has with a dataset via the medium of code. We want to help dig a "pit of success" where the least-effort path tends to lead towards success.
...
(Another framing of this principle is [Less Volume, More Creativity][less-volume], which comes from Mike McCarthy, the head coach of the Green Bay Parkers, and popularised in Statistics Education by [Randall Pruim][randy-pruim])
