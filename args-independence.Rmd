...
*   `rep()` has mutually exclusive `length.out` and `times` arguments. If you
    set them both, `length.out` silently wins.

    ```{r}
    rep(1:3, length.out = 3)
    rep(1:3, times = 3)
    ```
...
* Sections \@ref(args-mutually-exclusive) and \@ref(args-compound).

## How do I avoid?

Often these problems arise because when the functions were originally designed the scope was much smaller. It grew gradually over time, and no point did it seem worth the additional effort to refactor to a new design. This can make them hard to avoid. 

## How do I remediate?

Generally, you'll need to reconsider the complete interface. There are two common results:

* Splitting the function into multiple functions that each do one thing.

* Encapulsating related details arguments into a single object created by 
  a new function.

These are illustrated in the case studies below.

If these changes to the interface occur to exported functions in a package, you'll need to consider how to preserve the interface with deprecation warnings. For important functions, it is worth generating an message that includes new code to c?fcopy and paste.

### Case study: `fct_lump()` {#cs-fct-lump}

There are many different ways to decided how to lump uncommon factor levels together, and initially I attempted to encode these through arguments to `fct_lump()`. However, over time as the number of arguments increased, it gets harder and harder to tell what the options are. Now, we're growing `fct_lump()` capabilities by adding new functions, like `fct_lump_min()`.
...
* Both `n` and `prop` supplied: due to a bug in the code, this is treated
  the same way as both `n` and `prop` mmissing!
  
`ties.method()` only applies if either `n` or `prop`  is supplied.
