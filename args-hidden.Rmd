...
One common source of hidden arguments is the use of global options. These can be useful to control display but, as discussed in Chapter \@ref(args-user-defaults)), should not affect computation:
...
If you must depend on an environment variable or option, make sure it's an explicit argument, as in Chapter \@ref(args-user-defautlts). Such arguments generally should not affect computation (only side-effects like printed output or status messages); if they do affect results, follow Chapter \@ref(args-explain) to make sure to inform the user what's happening.
...
as.POSIXct <- function(x, tz = "") {
  base::as.POSIXct(as.POSIXlt(x, tz = tz), tz = tz)
}
```
...
This is an import argument coming from an environment variable, so we should also print it out if the user hasn't explicitly set it:
