...
* Have one argument for each part of the error that varies. Each argument is 
...
*   That a function returns the correct error type. This is a safer replacement 
    for using text matching because you don't need to worry about the test ]
    breaking just because the error message has been tweaked.s

    ```{r, eval = FALSE}
    expect_error(foofy(1, 2), class = "pkgname_error_bad_input")
    ```

*   If you want to be more specific, you should catch the error and then use 
    `expect_equal()` to compare components.
